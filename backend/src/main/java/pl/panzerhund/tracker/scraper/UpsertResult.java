 ...
 */
public record UpsertResult(Outcome outcome, boolean priceDropped, Instant previousLastSeenAt) {

    public enum Outcome { CREATED, UPDATED }

    static UpsertResult created() {
        return new UpsertResult(Outcome.CREATED, false, null);
    }

    static UpsertResult updated(boolean priceDropped, Instant previousLastSeenAt) {
        return new UpsertResult(Outcome.UPDATED, priceDropped, previousLastSeenAt);
    }
